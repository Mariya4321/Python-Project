...
def deal_card():  # choosing random cards
    card = [11, 2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 10, 10]
    cards = random.choice(card)
    return cards


def calculate_score(cards):  # Returning scores
    ...
    if sum(cards) == 21 and len(cards) == 2:
        return 0
    # Inside calculate_score() check for an 11 (ace). If the score is already over 21, remove the 11 and
...
def compare(user_score, computer_score):       # final display the winner
...
def play_game():        # all works and function calls
    ...
    # The score will need to be rechecked with every new card drawn and the checks in Hint 9 need to be
